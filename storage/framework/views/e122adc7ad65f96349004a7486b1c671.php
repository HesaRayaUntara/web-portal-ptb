            ...
            <div class="relative w-full overflow-hidden bg-gradient-to-br from-primary via-primaryDark to-secondary lg:w-2/5">
                ...
                <div class="relative flex flex-col justify-center p-5 text-white sm:p-6 lg:p-8">
                    <div class="space-y-2 sm:space-y-3">
                        ...
                        <h2 class="font-bold leading-tight sm:text-xl lg:text-2xl">Edit Admin</h2>
                        <p class="text-xs leading-relaxed text-white/90 sm:text-sm">Perbarui informasi akun admin untuk Program Studi Pemuliaan Tanaman dan Teknologi Benih.</p>
                    </div>
                    ...
                    <div class="mt-4 space-y-2 sm:mt-5 sm:space-y-2.5">
                        <div class="flex items-center gap-2 text-xs">
                            ...
                            <span>Update informasi akun</span>
                        </div>
                        <div class="flex items-center gap-2 text-xs">
                            <div class="flex h-6 w-6 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm sm:h-7 sm:w-7">
                                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span>Ubah password jika diperlukan</span>
                        </div>
                        <div class="flex items-center gap-2 text-xs">
                            <div class="flex h-6 w-6 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm sm:h-7 sm:w-7">
                                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="w-full max-w-xs space-y-5">
                    ...
                    <div class="space-y-1.5 text-center lg:text-left">
                        <div class="inline-flex items-center gap-1.5">
                            <h1 class="text-lg font-bold text-secondary sm:text-xl lg:text-2xl">Edit Admin</h1>
                        </div>
                    </div>
                    ...
                    <!-- Edit Form -->
                    <form class="space-y-3.5" method="POST" action="<?php echo e(route('admin.edit.submit', $admin->id)); ?>" id="editForm">
                        <?php echo csrf_field(); ?>
                        ...
                        <!-- Password Field -->
                        <div class="space-y-1">
                            <label for="password" class="block text-xs font-semibold text-textDark">Password Baru</label>
                            <div class="group relative">
                                <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2.5">
                                    <svg class="h-3.5 w-3.5 text-textMuted transition-colors group-focus-within:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <input id="password" name="password" type="password" placeholder="Masukkan password baru (opsional)" minlength="8" autocomplete="new-password" class="w-full rounded-lg border-2 border-gray-200 bg-gray-50 py-2 pl-9 pr-14 text-xs text-textDark placeholder:text-textMuted transition-all duration-200 focus:border-primary focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/10 sm:text-sm">
                            ...
                            <p class="text-xs text-textMuted">*kosongkan jika tidak ingin mengubah</p>
                        </div>

                        <!-- Password Confirmation Field -->
                        <div class="space-y-1">
                            <label for="password_confirmation" class="block text-xs font-semibold text-textDark">Konfirmasi Password Baru</label>
                            <div class="group relative">
                                <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2.5">
                                    <svg class="h-3.5 w-3.5 text-textMuted transition-colors group-focus-within:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <input id="password_confirmation" name="password_confirmation" type="password" placeholder="Ulangi password baru" minlength="8" autocomplete="new-password" class="w-full rounded-lg border-2 border-gray-200 bg-gray-50 py-2 pl-9 pr-14 text-xs text-textDark placeholder:text-textMuted transition-all duration-200 focus:border-primary focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/10 sm:text-sm">
                        ...
                        <button type="submit" id="submitBtn" class="group relative w-full overflow-hidden rounded-lg bg-primary hover:bg-primaryDark py-2 px-4 text-center text-sm font-semibold text-white">
                            <span class="relative z-10 flex items-center justify-center gap-2"><span>Simpan Perubahan</span></span>
                        </button>
                    </form>
                </div>
            </div>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Password Match Validation (only if password is filled)
            ...
            function validatePasswordMatch() {
                if (passwordField.value && passwordConfirmField.value) {
                    ...
                    // Both empty is OK (optional password change)
                    passwordConfirmField.required = false;
                    passwordConfirmField.setCustomValidity('');
                    passwordConfirmField.classList.remove('border-red-300', 'bg-red-50');
                } else if (passwordField.value && !passwordConfirmField.value) {
                    // Password filled but confirmation empty - make it required
                    passwordConfirmField.required = true;
                    passwordConfirmField.setCustomValidity('Konfirmasi password wajib diisi jika password diisi');
                    passwordConfirmField.classList.add('border-red-300', 'bg-red-50');
                } else if (!passwordField.value && passwordConfirmField.value) {
                    // Confirmation filled but password empty - clear confirmation
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\web-portal-ptb\resources\views/halaman-admin/admin-edit.blade.php ENDPATH**/ ?>