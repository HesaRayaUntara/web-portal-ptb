...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row">
        ...
        <main class="flex-1 p-6 lg:p-8">
            ...
            {{-- Form Edit Publikasi --}}
            <form method="POST" action="{{ route('admin.dosen.updatePublikasi', $publikasi->id) }}" class="rounded-xl border border-borderSoft bg-white p-6 shadow-soft">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="dosen_id" name="dosen_id"
                            ...
                            @foreach($dosen as $d)
                                <option value="{{ $d->id }}" {{ old('dosen_id', $publikasi->dosen_id) == $d->id ? 'selected' : '' }}>{{ $d->nama }}</option>
                            @endforeach
                        </select>
                        @error('dosen_id')
