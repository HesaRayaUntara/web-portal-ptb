...
@section('title', 'Admin - Edit HKI/Paten')
...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row lg:items-stretch">
        ...
        <main class="flex-1 p-6 lg:p-8">
            ...
            @if(isset($hki) && $hki)
            {{-- Form Edit HKI --}}
            <form method="POST" action="{{ route('admin.dosen.updateHki', $hki) }}" class="rounded-xl border border-borderSoft bg-white p-6 shadow-soft">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="dosen_id" name="dosen_id"
                            ...
                            @foreach($dosen as $d)
                                <option value="{{ $d->id_dosen }}" {{ old('dosen_id', $hki->dosen_id) == $d->id_dosen ? 'selected' : '' }}>{{ $d->nama }}</option>
                            @endforeach
                        </select>
                        @error('dosen_id')
                    ...
                    <div>
                        ...
                        <input type="text" id="judul_karya" name="judul_karya" value="{{ old('judul_karya', $hki->judul_karya) }}" required
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                    ...
                    <div>
                        ...
                        <select id="jenis_karya" name="jenis_karya" required
                            ...
                            @foreach($jenisKarya as $jk)
                                <option value="{{ $jk->j_karya }}" {{ old('jenis_karya', $hki->jenis_karya) == $jk->j_karya ? 'selected' : '' }}>{{ $jk->j_karya }}</option>
                            @endforeach
                        </select>
                        @error('jenis_karya')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" id="tahun" name="tahun" value="{{ old('tahun', $hki->tahun) }}" required min="1900" max="{{ date('Y') }}"
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
