...
@section('title', 'Admin - Edit Pengabdian Masyarakat')
...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row lg:items-stretch">
        ...
        <main class="flex-1 p-6 lg:p-8">
            ...
            <form method="POST" action="{{ route('admin.dosen.updatePengabdian', $pengabdian) }}" class="rounded-xl border border-borderSoft bg-white p-6 shadow-soft">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="dosen_id" name="dosen_id"
                            ...
                            @foreach($dosen as $d)
                                <option value="{{ $d->id_dosen }}" {{ old('dosen_id', $pengabdian->dosen_id) == $d->id_dosen ? 'selected' : '' }}>{{ $d->nama }}</option>
                            @endforeach
                        </select>
                        @error('dosen_id')
                    ...
                    <div>
                        ...
                        <input type="text" id="judul_pengabdian" name="judul_pengabdian" value="{{ old('judul_pengabdian', $pengabdian->judul_pengabdian) }}" required
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                    ...
                    <div>
                        ...
                        <input type="number" id="tahun" name="tahun" value="{{ old('tahun', $pengabdian->tahun) }}" required min="1900" max="{{ date('Y') }}"
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
