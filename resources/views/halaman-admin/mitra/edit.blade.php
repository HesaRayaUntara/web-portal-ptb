...
@section('title', 'Admin - Edit Mitra')
...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row">
        ...
        <main class="flex-1 p-6 lg:p-8">
            ...
            <form method="POST" action="{{ route('admin.mitra.update', $mitra) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="nama_mitra" name="nama_mitra" required
                                ...
                                placeholder="Masukkan nama mitra" value="{{ old('nama_mitra', $mitra->nama_mitra) }}">
                        </div>
                        <div>
                            <label for="logo" class="mb-2 block text-sm font-semibold text-textDark">Logo</label>
                            @if($mitra->logo)
                                <div class="mb-2">
                                    <img src="{{ Storage::url($mitra->logo) }}" alt="Logo Mitra" class="h-16 w-auto rounded-lg border border-borderSoft object-cover">
                                </div>
                            @endif
