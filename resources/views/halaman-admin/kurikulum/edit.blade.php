...
@section('title', 'Admin - Edit Kurikulum')
...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row">
        ...
        <main class="flex-1 p-6 lg:p-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-textDark">Edit Mata Kuliah</h1>
                <p class="mt-1 text-sm text-textMuted">Edit informasi mata kuliah</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.kurikulum.update', $kurikulum) }}" class="rounded-xl border border-borderSoft bg-white p-6 shadow-soft">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="kode_mk" name="kode_mk" value="{{ old('kode_mk', $kurikulum->kode_mk) }}" required
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_mk" name="nama_mk" value="{{ old('nama_mk', $kurikulum->nama_mk) }}" required
                            class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                    ...
                    <div>
                        ...
                        <select id="jenis_mk" name="jenis_mk" required
                            ...
                            <option value="CCC" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'CCC' ? 'selected' : '' }}>CCC: Common Core Courses</option>
                            <option value="FC" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'FC' ? 'selected' : '' }}>FC: Foundational Courses</option>
                            <option value="FL" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'FL' ? 'selected' : '' }}>FL: Foundational Literacies Courses</option>
                            <option value="IC" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'IC' ? 'selected' : '' }}>IC: In-depth Study Program Courses</option>
                            <option value="ACC" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'ACC' ? 'selected' : '' }}>ACC: Vocational Core Courses</option>
                            <option value="EC" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'EC' ? 'selected' : '' }}>EC: Enrichment Courses</option>
                            <option value="FYP" {{ old('jenis_mk', $kurikulum->jenis_mk) == 'FYP' ? 'selected' : '' }}>FYP: Final Year Project</option>
                        </select>
                        @error('jenis_mk')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <input type="number" id="sks_kuliah" name="sks_kuliah" value="{{ old('sks_kuliah', $kurikulum->sks_kuliah) }}" required min="0"
                                class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                                placeholder="0">
                            @error('sks_kuliah')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="number" id="sks_praktikum" name="sks_praktikum" value="{{ old('sks_praktikum', $kurikulum->sks_praktikum) }}" required min="0"
                                class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                                placeholder="0">
                            @error('sks_praktikum')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
