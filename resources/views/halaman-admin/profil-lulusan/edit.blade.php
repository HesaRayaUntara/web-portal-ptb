...
@section('title', 'Admin - Edit Profil Lulusan')
...
<div class="rounded-section border border-borderSoft bg-white shadow-soft">
    <div class="flex flex-col gap-8 lg:flex-row">
        ...
        <main class="flex-1 p-6 lg:p-8">
            ...
            @if(isset($profilLulusan) && $profilLulusan)
            <form method="POST" action="{{ route('admin.profil-lulusan.update', $profilLulusan) }}" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="peran" name="peran" required
                                ...
                                placeholder="cth. Peneliti Pemuliaan Tanaman" value="{{ old('peran', $profilLulusan->peran) }}">
                        </div>
                        <div>
                            ...
                            <textarea id="deskripsi_kemampuan" name="deskripsi_kemampuan" rows="5" required
                                class="w-full rounded-xl border border-borderSoft px-4 py-3 text-sm text-textDark placeholder:text-textMuted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                                placeholder="Masukkan deskripsi kemampuan">{{ old('deskripsi_kemampuan', $profilLulusan->deskripsi_kemampuan) }}</textarea>
                        </div>
                    </div>
                </section>
