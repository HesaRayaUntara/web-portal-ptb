...
<section class="mt-8 rounded-section bg-white p-6 shadow-soft md:mt-6 md:p-8">
    <div class="space-y-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <a href="{{ route('galeri') }}" class="inline-flex items-center gap-2 rounded-full border border-primary/20 px-4 py-1.5 text-xs font-semibold text-primary transition hover:border-primary hover:shadow-soft">
                Lihat semua galeri
                ...
                <div class="carousel-track flex transition-transform duration-500 ease-in-out" id="carouselTrack">
                    ...
                    <div class="carousel-slide min-w-full flex-shrink-0" data-index="{{ $index }}">
                        <div class="relative h-48 md:h-64 overflow-hidden rounded-card">
                            <img src="{{ $photo['image'] }}" alt="{{ $photo['title'] }}" class="h-full w-full object-cover" loading="{{ $index === 0 ? 'eager' : 'lazy' }}">
...
<section class="mt-8 rounded-section bg-white p-6 shadow-soft md:mt-6 md:p-6 lg:p-8">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <a href="{{ route('berita') }}" class="inline-flex items-center gap-2 rounded-full border border-primary/20 px-4 py-1.5 text-xs font-semibold text-primary transition hover:border-primary hover:shadow-soft">
        ...
        <a href="{{ route('berita.detail', $item->slug) }}" class="group block">
            <article class="flex flex-col overflow-hidden rounded-card border border-primary/10 bg-white shadow-soft transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-card md:flex-row">
                <div class="relative h-24 w-full flex-shrink-0 md:h-32 md:w-28">
                    <img src="{{ $item->image ? Storage::url($item->image) : 'https://via.placeholder.com/800x600' }}" alt="{{ $item->judul }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" loading="lazy">
                ...
                <div class="flex flex-1 flex-col justify-between space-y-2 p-3">
                    <div>
                        <h3 class="text-sm font-semibold text-textDark group-hover:text-primary line-clamp-1 md:text-base">{{ $item->judul }}</h3>
                        <p class="mt-1 text-xs text-textMuted line-clamp-2 md:text-sm md:line-clamp-3">{{ Str::limit(strip_tags($item->isi), 200) }}</p>
                    </div>
    ...
    <div class="mt-4 overflow-hidden">
        <div class="mitra-scroll-container flex gap-2 md:gap-3">
            ...
            <div class="mitra-item flex min-w-[80px] items-center justify-center overflow-hidden rounded-lg bg-white p-2 transition hover:border-primary/20 hover:shadow-soft md:min-w-[100px] md:p-2.5">
                <img src="{{ Storage::url($item->logo) }}" alt="404 not found" class="h-auto max-h-8 w-full object-contain md:max-h-10">
            </div>
            @endforeach
...
{{-- Script untuk Profil Lulusan Accordion --}}
@if(isset($profilProdi) && $profilProdi->profilLulusan && $profilProdi->profilLulusan->count() > 0)
<script>
    document.addEventListener('DOMContentLoaded', function() {
...
{{-- CSS untuk animasi scroll mitra --}}
@if(isset($mitra) && $mitra->count() > 0)
<style>
    @keyframes scroll {
        0% {
