            ...
            <div class="relative">
                <div class="inline-flex w-full items-center justify-end md:justify-start">
                    <button type="button" id="categoryFilterBtn"
                        ...
                        <span id="selected-category">{{ request('kategori') ?: 'Semua Kategori' }}</span>
                    ...
                    <div id="categoryDropdown" class="absolute right-0 top-full z-50 mt-2 hidden min-w-[180px] flex-col rounded-badge border border-primary/10 bg-white text-sm text-textDark shadow-soft">
                        <a href="{{ route('berita') }}"
                           class="flex items-center gap-3 rounded-badge px-4 py-2 transition hover:bg-primary/10 hover:text-primary {{ !request('kategori') ? 'bg-primary/10 text-primary font-semibold' : '' }}">
                            <span>Semua Kategori</span>
                        </a>
                        @foreach ($categories as $category)
                            <a href="{{ route('berita', ['kategori' => $category]) }}"
                               class="flex items-center gap-3 rounded-badge px-4 py-2 transition hover:bg-primary/10 hover:text-primary {{ request('kategori') === $category ? 'bg-primary/10 text-primary font-semibold' : '' }}">
                                <span>{{ $category }}</span>
                            </a>
                        @endforeach
                    </div>
            ...
            @foreach ($news as $item)
                <a href="{{ route('berita.detail', $item->slug) }}" class="group block h-full">
                    <article class="flex h-full flex-col overflow-hidden rounded-card border border-primary/10 bg-white shadow-soft transition hover:-translate-y-1 hover:border-primary/30 hover:shadow-card">
                        <div class="relative">
                            <img src="{{ $item->image ? Storage::url($item->image) : 'https://via.placeholder.com/800x600' }}" alt="{{ $item->judul }}" class="h-32 w-full object-cover" loading="lazy">
                            @if($item->kategori)
                        ...
                        <div class="flex flex-1 flex-col justify-between space-y-2 p-4">
                            <div>
                                <h3 class="text-lg font-semibold text-textDark group-hover:text-primary line-clamp-2">{{ $item->judul }}</h3>
                                <p class="mt-2 text-xs text-textMuted leading-relaxed line-clamp-2">{{ Str::limit(strip_tags($item->isi), 100) }}</p>
                            </div>
